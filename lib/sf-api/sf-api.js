 ...
 */
function find_suppressions(req, callback) {
    var url = get_sf_url('suppressions');
    request.json_get(req, url, {limit: 0}, function(err, response, body) {
        callback(err, body);
    });
}

function find_suppression(req, suppression_id, callback) {
    var url = get_sf_url('suppressions');
    request.json_get(req, url, {suppression_id: suppression_id}, function(err, response, body) {
        if (err) {
            callback(err);
        }
        else {
...
function get_rowitem_content(req, rowitem_uuid, callback) {
        ...
        if (err) {
            ...
            xml.parseString(body.content, options, function(err, result) {
                if (err) {
                    ...
                    //console.log(result.processitem.handlelist.handle);
                    //console.log(body.content);
                    //console.log(result);
                    //console.log(util.inspect(result, false, null));
                    // Replace the XML content with JSON.
                    body.content = result;
                    callback(null, body);
                }
            });
...
function get_hosts_by_hash_list(req, hash_list, callback) {
    ...
    request.form_post(req, url, form, function (err, response, body) {
        if (err) {
            callback(err);
        }
        else {
    ...
    async.waterfall(
        [
            function (callback) {
                ...
                request.form_post(req, url, params, callback);
            },
