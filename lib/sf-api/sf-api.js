 ...
 * @param req - the request.
 * @param clusters - a comma separated list of clusters.
 * @param callback - function(err, clusters)
 */
function get_acquisitions(req, clusters, callback) {
    var url = get_ss_url('api/v1/acquisition/');
    request.json_get(req, url, {clusters: clusters}, function(err, response, body) {
        callback (err, body);
    });
}
