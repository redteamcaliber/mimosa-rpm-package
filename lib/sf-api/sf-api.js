        ...
        if (err) {
            ...
            if (!body.state) {
                ...
                async.series(
                    [
                        function (callback) {
                            // Post a comment to the hit recording the acquisition.
                            post_comment(params.rowitem_uuid, comment, attributes, callback);
                        },
                        function (callback) {
...
function get_acquisitions_by_id(ids, attributes, callback) {
    if (ids && ids.length > 0) {
        ...
        request.json_get(url, {}, attributes, function(err, response, body) {
            err ? callback(err) : callback(null, body && body.objects ? body.objects : []);
        });
    }
    else {
        // Nothing to process.
