...
function get_usersettings(attributes, callback) {
...
function post_usersettings(body, attributes, callback) {
    var url = get_sf_url('usersettings');
...
function delete_usersettings(attributes, callback) {
    request.json_delete(get_sf_url('usersettings'), attributes, function (err, response, body) {
        callback(err, body);
    });
}
 ...
 */
function find_suppressions(attributes, callback) {
...
function find_suppression(suppression_id, attributes, callback) {
    var url = get_sf_url('suppressions');
    request.json_get(url, {suppression_id: suppression_id}, attributes, function(err, response, body) {
        if (err) {
            ...
            if (data) {
                ...
                callback('Unable to parse suppression response: ' + body);
            }
        }
    });
}
...
function get_sf(req, callback) {
    ...
    request.json_get(url, {}, req.attributes, function(err, response, body) {
        callback(err, body);
    });
}
...
function post_sf(req, callback) {
    var url = get_sf_url(req.originalUrl.substring('/sf/api'.length, req.originalUrl.length));
    request.json_post(url, req.body, req.attributes, function(err, response, body) {
        callback(err, body);
    });
}
...
function delete_sf(req, callback) {
    var url = get_sf_url(req.originalUrl.substring('/sf/api'.length, req.originalUrl.length));
    request.json_delete(url, req.attributes, function(err, response, body) {
        callback(err, body);
    });
}
 ...
 * @param callback - function(err, body)
 */
function post_acquisition(cluster_uuid, hash, params, attributes, callback) {
    async.waterfall(
        [
            function (callback) {
                ...
                request.form_post(url, params, attributes, callback);
            },
            ...
            function (body, callback) {
                ...
                post_comment(params.rowitem_uuid, attributes, comment, function (err) {
                    err ? callback(err) : callback(null, body);
                });
            },
            function (body, callback) {
                // Tag the row.
                post_tag(params.rowitem_uuid, attributes, 'investigating', function (err) {
                    err ? callback(err) : callback(null, body);
                });
            }
        ],
 ...
 * @param callback - function(err, audit).
 */
...
// Suppressions.
exports.find_suppression = find_suppression;
exports.find_suppressions = find_suppressions;
