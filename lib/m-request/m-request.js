...
function json_get(req, url, callback) {
    request(req, {
        method: 'GET',
        url: url,
        json: true
...
function request(req, options, callback) {
    ...
    var url = options.url ? options.url : options.uri;
    if (!url) {
    ...
    async.waterfall(
        [
            ...
            function (ticket, callback) {
                // Obtain a cookie jar.
                var jar = options.jar ? options.jar : request_api.jar();
                ...
                try {
                    ...
                    request_api(options, function (err, response, body) {
                        log.debug(_.sprintf('Invoked url: %s %s in %s ms', options.method, url, Date.now() - start_time));

                        if (err) {
                            // Unknown error.
                            log.error('Unknown error while invoking url: %s %s', options.method, url);
                            if (body) {
                                log.error(body)
                            }
                            callback(err);
                            ...
                            if (body) {
                                ...
                                callback(_.sprintf('Response body is undefined while invoking url: %s %s',
                                    options.method, response.request.uri.href))
                            }
                        }
                        else {
                            // Error, bad HTTP status code.
                            var message = _.sprintf('Bad http response code (%s) while invoking url: %s %s',
                                response.statusCode, options.method, response.request.uri.href);
                            log.error(message);
                            if (body) {
                                log.error(body)
                            }
                    ...
                    // Error
                    callback('Exception making request: - ' + e.stack);
                }
            }
        ],
        // Invoke the callback.
        ...
        if (err) {
            ...
            if (data.result == 1 && data.ticket) {
                try {
                    ...
                    // Error
                    log.error('Exception while refreshing sso ticket.');
                    log.error(e.stack);
                    callback(e);
                }
            }
                ...
                if (message) {
                    ...
                    callback('Error: Unable to obtain SSO ticket, no message returned.');
                }
            }
        }
    });
