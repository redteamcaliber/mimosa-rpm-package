...
function require_authentication(settings) {
    ...
    console.log('Loading key file: %s', pubkey_setting);
        ...
        if (!settings.auth_cookie) {
            ...
            parse_ticket(ticket, function(err, ticket_data) {
                if (err) {
                    ...
                    if (req.session && req.session.uid && req.session.uid != ticket_data.uid) {
                        // Error, session user does not match the ticket data.
                        log.error('UID session mismatch error.  Session: %s, Ticket: %s', req.session.uid, ticket_data.uid);
                        ...
                        if (!is_auth_token_valid(settings.auth_token, ticket_data)) {
                            ...
                            req.attributes.ticket = ticket;

                            if (req.session) {
                                // Save the user's uid to the session.
                                req.session.uid = ticket_data.uid;

                                // Set the session expiration to the value from the ticket.
                                req.session.cookie.expires = moment.unix(ticket_data.validuntil).toDate();
                            }
