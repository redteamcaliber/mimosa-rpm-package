...
function parse_ticket(ticket, callback) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        if (pair) {
            ...
            log.warn(_.sprintf('Unable to parse ticket: %s', ticket));
            return callback(_.sprintf('Unable to parse ticket data: %s', token)); // **EXIT**
        }
    }
...
function get_login_url(req, settings, refresh) {
    try {
        ...
        var back_url = _.sprintf('%s://%s%s', req.protocol, req.host, req.url);
...
function require_authentication(settings) {
    ...
    return function (req, res, next) {
        if (!settings.auth_cookie) {
            return next(_.sprintf('Error: %s setting is not valid.', 'auth_cookie'));
        }
        else if (!settings.auth_url) {
            return next(_.sprintf('Error: %s setting is not valid.', 'auth_url'));
        }

        var cookies = req.cookies;
        if (!cookies) {
            // Error, cookies are not enabled.
            log.error('Cookies are not enabled.');
            return res.redirect(get_login_url(req, settings));
        }
        else if (!cookies[settings.auth_cookie]) {
            // No sso cookie found, redirect to the login page.
            return res.redirect(get_login_url(req, settings));
        }
        else {
            // Auth cookie was found, validate its value.
            var ticket = cookies[settings.auth_cookie];
            ...
            parse_ticket(ticket, function(err, ticket_data) {
                if (err) {
                    ...
                    if (is_signature_valid(pubkey, ticket_data)) {
                        ...
                        var message = _.sprintf('Invalid ticket signature for user: %s', ticket_data.uid);
                        log.error(message);
...
/**
 * Optimized function to parse a uid out of a ticket.
 * @param ticket - the ticket.
 * @returns {*} the uid or null if the ticket could not be parsed.
 */
function parse_uid(ticket) {
    try {
        var uid_start_index = ticket.indexOf('uid=');
        var uid_end_index = ticket.indexOf(';', uid_start_index);
        var uid = ticket.substring(uid_start_index + 4, uid_end_index);
        log.debug(uid);
        return uid;
    }
    catch (e) {
        log.warn('Unable to parse ticket: ' + ticket);
        return null;
    }
}
