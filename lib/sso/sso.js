        ...
        if (auth_token) {
            ...
            if (ticket_data.tokens) {
                ...
                if (_.indexOf(user_app_tokens, auth_token) != -1) {
                    ...
                    // Log the failure.
                    log.warn(_.sprintf('User: %s attempting access but does not have authtoken: %s',
...
function send_unauth(req, res, settings) {
    if (req.xhr) {
        ...
        // Error, redirect to the sso page.
        res.redirect(settings.get(settings.SSO_UNAUTH_URL));
    }
}
                ...
                if (err) {
                    ...
                    if (is_signature_valid(pubkey, ticket_data)) {
                        if (!is_auth_token_valid(settings.auth_token, ticket_data)) {
                            ...
                            log.warn(_.sprintf('User: %s does not have the required auth token.', ticket_data.uid));
