...
app.get('/suppressions', function (req, res, next) {
    sf_api.find_suppressions(req.attributes, function (err, suppressions) {
        if (err) {
            next(err);
        }
        else {
...
app.get('/suppressions/:suppression_id', function (req, res, next) {
    ...
    if (!suppression_id) {
        ...
        var suppression = sf_api.find_suppression(suppression_id, req.attributes, function (err, suppression) {
            if (err) {
                next(err);
            }
            else {
...
app.post('/api/acquire', function (req, res, next) {
    ...
    if (route_utils.validate_input('am_cert_hash', 'file_name', 'file_path', 'method', 'user', 'password')) {
