...
app.get('/api/ioc-summary', function(req, res, next) {
    if (!req.query.services) {
        res.send(400, '"services" is required.');
    }
    else if (!req.query.clusters) {
        res.send(400, '"clusters" is required.');
    }
    else {
        req.query.limit = 0; // Retrieve all records.
        // TODO: Remove this after api fix.
        req.query['services[]'] = req.query.services;
        req.query['clusters[]'] = req.query.clusters;
        sf_api.get_ioc_summary(req, req.query, function(err, body) {
...
app.post('/api/suppressions', function(req, res, next) {
    // TODO: Should validate the body if this code stays.
    sf_api.post_suppression(req, req.body, function(err, body) {
        if (err) {
            next(err);
        }
        else {
