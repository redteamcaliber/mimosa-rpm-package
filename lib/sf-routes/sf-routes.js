...
app.get('/audit/file-item/:rowitem_uuid', function(req, res, next) {
    ...
    if (!rowitem_uuid) {
        ...
        sf_api.get_rowitem_content(req, rowitem_uuid, function(err, response) {
            var context = route_utils.default_context(req);
            context.fileitem = response.content.fileitem;
            context.get_array = function(item) {
            ...
            //context.usersettings = route_utils.stringify(results[0]);
            //context.clusters = route_utils.stringify(results[1]);
            route_utils.render_template(res, '/sf/audit/file-item.html', context, next);
        });
    }
});
