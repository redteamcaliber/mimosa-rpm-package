 ...
 * @param response - the response object containing the rowitem_type and XML content.
 * @param callback - function(err, content)
 */
function construct_audit_content(response, callback) {
    var type = response.rowitem_type.toLowerCase();
    var template_file = AUDIT_TEMPLATE_MAP[type];
    if (!template_file) {
        callback('Unable to locate template for type: ' + type);
    }
    else {
        ...
        sf_api.get_rowitem_content(req, rowitem_uuid, function (err, response) {
            if (err) {
                ...
                if (!response) {
                    ...
                    construct_audit_content(response, function (err, content) {
                        if (err) {
                            next(err);
                        }
                        else {
...
/**
 * Retrieve the audit HTML output based on an XML input value from the post.
 */
app.post('/api/audit', function (req, res, next) {
    var body = req.body;

    if (!req.body.content) {
        res.send(400, '"content" is required.');
    }
    else {
        // Get the XML type.
        ...
        xml.parseString(body.content, options, function(err, result) {
            if (err) {
                ...
                console.dir(result.persistenceitem.fileitem.peinfo.importedmodules.module);
                ...
                construct_audit_content(input, function (err, content) {
                    if (err) {
                        next(err);
                    }
                    else {
