    ...
    async.parallel(
        ...
        function (err, results) {
            if (err) {
                ...
                var clients = _.values(client_map);
                var context = route_utils.default_context(req);
                context.services = route_utils.stringify(services);
                context.clients = route_utils.stringify(clients);
        ...
        if (isNaN(req.params.suppression_id)) {
            ...
            sf_api.get_suppression(req.params.suppression_id, req.attributes, function(err, suppression) {
                if (err) {
                    ...
                    // Display the template.
                    var context = route_utils.default_context(req);
                    context.suppression = route_utils.stringify([suppression] || []);
...
app.get('/acquisitions', function (req, res, next) {
    sf_api.get_clusters(req.attributes, function(err, clusters) {
        if (err) {
            next(err);
        }
        else {
