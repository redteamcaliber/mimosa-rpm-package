...
app.get('/hits', function (req, res, next) {
    sf_api.get_services_clients_clusters(req.attributes, function (err, results) {
        if (err) {
            ...
            route_utils.render_template(res, '/sf/hit-review.html', context, next);
        }
    });
});
...
var is_mcube_configured = settings.get('uac:mcube_api_url') && settings.get('uac:mcube_api_key');

function fill_md5_details(source, md5_property, callback) {
    if (is_mcube_configured) {
...
//
// Retrieve the list of FE alert summary items.
//
app.get('/api/summary', function(req, res, next) {
    sf_api.get_ioc_summary_v2(req.query, req.attributes, function(err, list) {
        if (err) {
            next(err);
        }
        else {
